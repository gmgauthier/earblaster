...
Current release: **[0.2.2](https://github.com/gmgauthier/earblaster/releases/tag/v0.2.2)** (2026-09-15).
...
**M0–M6 are in the tree.** Window, spin, sound, playlist, cover, EQ/prefs/keyboard, and packaging (`debian/`, `scripts/release.sh`). Tagged **0.2.2** (Thunar Open with; stale M3Us resolve; AppImage uses only bundled GStreamer; do not mix with the host).
...
```
sudo apt install ./earblaster_0.2.2-1_amd64.deb
```
...
```
chmod +x EarBlaster-0.2.2-x86_64.AppImage
./EarBlaster-0.2.2-x86_64.AppImage
```
...
```
./EarBlaster-0.2.2-x86_64.AppImage --appimage-extract
