...
bool already_has(const std::vector<SyncDevice>& devices, const Glib::RefPtr<Gio::File>& file)
{
  if (!file)
    return false;
...
void add_device(std::vector<SyncDevice>& devices, const Glib::RefPtr<Gio::File>& root,
...
std::vector<SyncDevice> list_sync_devices()
{
  ...
  std::stable_partition(devices.begin(), devices.end(), [](const SyncDevice& d) {
